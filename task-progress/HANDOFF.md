...
## 🏁 Current Status: "The Viral Shell is Ready, The Engine is Still Smoking"
...
## 🔑 Critical API & Environment State
...
| **VAPID Keys** | Pending | `.env` | Required for push notifications (`usePushNotifications.ts`). |
| **Google Maps** | N/A | Local | Using `MapLibre` + OpenStreetMap (no API key required currently). |

---

## 🔥 Brutally Honest: Critical Blockers & "Messy" Areas
...
2. **React Dispatcher / Performance Hook Crashes**:
   - `src/lib/utils/performance.ts` was attempting to use React hooks (`useEffect`, `useRef`) in a static utility class.
   - **Status**: I removed the `usePerformance` hook, but logs still suggest `performance.ts` is called in invalid contexts.
...
3. **Auth Redirect Loop/404**:
   ...
   - **Status**: Updated `Login.tsx` to redirect to `/map`. Verified that `/` redirects to `/map` if logged in.
...
| :--- | :--- | :--- |
| **Landing/Onboarding** | 🟢 100% | Looks premium, high conversion, feels "viral". |
| **Live Map (HUD)** | 🟡 85% | Renders well, real proximity works, but territory ownership UI is slightly janky. |
| **Social Grid** | 🟡 70% | High energy, but 404s on refresh and card spacing is tight. |
| **Dating (Hyper-Match)**| 🟢 90% | RPC `find_best_matches` is live and functional. |
| **Economy (HUP Bits)** | 🔴 40% | DB tables exist, but Stripe Connect onboarding is blocked by deployment. |
| **Streaming** | 🟡 60% | LiveKit integration code is there, but signaling needs a heavy load test. |
