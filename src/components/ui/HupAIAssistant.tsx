    ...
    const handleAutomationCommand = async (command: AutomationCommand) => {
        ...
        if (!consumeEnergy(3)) {
            setMessages(prev => [...prev, {
                ...
                role: 'system',
                content: "Warning: Low Energy core. Please wait for replenishment before initiating automation commands."
            }]);
            return;
        }
        ...
        try {
            // Call AI Hub to execute automation command
            const { data, error } = await supabase.functions.invoke('hup-ai-hub', {
                body: {
                    prompt: `Execute automation command: ${command.name} with parameters ${JSON.stringify(command.parameters)}`,
                    feature: 'automation',
            ...
            // Display automation result in chat
            setMessages(prev => [...prev, {
                id: Date.now().toString(),
                role: 'system',
                content: `Executing automation: ${command.name}\n${data.content}`
            }]);
            ...
            setMessages(prev => [...prev, {
                id: 'error',
                role: 'system',
                content: `Automation command failed: ${error.message || 'Unknown error.'}`
            }]);
        } finally {
            setIsDataLoading(false);
        }
                            ...
                            <>
                                ...
                                <div className="p-4 border-t border-white/5 bg-black/40">
                                    ...
                                    <div className="mt-3 border-t border-white/5 pt-3">
                                        <p className="text-[9px] font-black uppercase tracking-widest text-primary mb-2">Automation Commands</p>
                                        <div className="grid grid-cols-2 gap-2">
                                            {automationCommands.map(command => (
                                                <button
                                                    key={command.id}
                                                    onClick={() => handleAutomationCommand(command)}
                                                    className="text-[8px] font-black uppercase tracking-widest text-muted-foreground hover:text-primary transition-colors hover:scale-105 text-left"
                                                >
                                                    <div className="flex items-center gap-1">
                                                        <Zap className="w-2 h-2" />
                                                        {command.name}
                                                    </div>
                                                    <p className="text-[7px] font-normal text-muted-foreground mt-1">{command.description}</p>
                                                </button>
                                            ))}
                                        </div>
                                    </div>
