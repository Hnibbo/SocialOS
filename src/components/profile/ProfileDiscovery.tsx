...
export const ProfileDiscovery: React.FC<{ userId?: string }> = ({ userId }) => {
    ...
    const [stats, setStats] = useState({ followers: 0, following: 0, posts: 0, matches: 0 });
    const [activity, setActivity] = useState({ content: [], activities: [] });
    const [isFollowing, setIsFollowing] = useState(false);
    const { user: currentUser } = useAuth();
    const { fetchUserStats, fetchUserActivity, checkFollowingStatus, followUser, unfollowUser } = useProfiles();
    ...
    const loadProfile = async () => {
        try {
            ...
            if (profileData) {
                ...
                // Fetch user stats
                const userStats = await fetchUserStats(targetId);
                setStats(userStats);

                // Fetch user activity
                const userActivity = await fetchUserActivity(targetId);
                setActivity(userActivity);

                // Check following status
                if (currentUser?.id && currentUser.id !== targetId) {
                    const following = await checkFollowingStatus(currentUser.id, targetId);
        ...
        <div className="max-w-4xl mx-auto p-4 space-y-8">
                            ...
                            <GlassCard className="p-4 bg-white/5 border-white/5">
                                ...
                                <div className="space-y-4">
                                    <div className="flex items-center justify-between">
                                        ...
                                        <span className="font-black text-primary">{stats.followers.toLocaleString()}</span>
                                    </div>
                                    <div className="flex items-center justify-between">
                                        <span className="text-xs font-bold text-muted-foreground">Following</span>
                                        <span className="font-black text-primary">{stats.following.toLocaleString()}</span>
                                    </div>
                                    <div className="flex items-center justify-between">
                                        <span className="text-xs font-bold text-muted-foreground">Posts</span>
                                        <span className="font-black text-primary">{stats.posts.toLocaleString()}</span>
                                    </div>
                                    <div className="flex items-center justify-between">
                                        <span className="text-xs font-bold text-muted-foreground">Matches</span>
                                        <span className="font-black text-primary">{stats.matches.toLocaleString()}</span>
                                    </div>
                                </div>
                            </GlassCard>
            ...
            <div className="space-y-6 mt-8">
                ...
                {activity.content.length > 0 && (
                    <div className="space-y-4">
                        <h3 className="text-xs font-black uppercase tracking-widest opacity-30">Recent Posts</h3>
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                            {activity.content.slice(0, 6).map((post: any) => (
                                <GlassCard key={post.id} className="p-4 hover:border-primary/50 transition-all">
                                    {post.thumbnail_url && (
                                    ...
                                    <p className="text-sm line-clamp-2">{post.text_content}</p>
                ...
                {activity.activities.length > 0 && (
                    <div className="space-y-4">
                        <h3 className="text-xs font-black uppercase tracking-widest opacity-30">Recent Activities</h3>
                        <div className="space-y-3">
                            {activity.activities.map((activity: any) => (
                                <GlassCard key={activity.id} className="p-4 hover:border-primary/50 transition-all">
                                    <div className="flex items-start justify-between">
                                        <div className="flex-1">
                                            ...
                                            <div className="flex items-center gap-3 mt-2 text-xs text-muted-foreground">
                                                ...
                                                <span>{activity.current_attendees}/{activity.max_attendees} attendees</span>
                                            </div>
                                        </div>
                                        <div className="ml-4">
                                            {activity.is_free ? (
                                                <span className="px-2 py-1 rounded-full bg-green-500/20 text-green-500 text-[10px] font-bold uppercase">
                                                    Free
                                                </span>
                                            ) : (
                                                <span className="px-2 py-1 rounded-full bg-yellow-500/20 text-yellow-500 text-[10px] font-bold uppercase">
                                                    Paid
                                                </span>
                                            )}
                                        </div>
                ...
                {/* No Activity */}
                {activity.content.length === 0 && activity.activities.length === 0 && (
                    <div className="text-center py-12">
                        <Sparkles className="w-12 h-12 mx-auto text-muted-foreground opacity-50" />
                        <p className="mt-4 text-muted-foreground">No activity yet. Start sharing content or joining activities!</p>
                    </div>
                )}
            </div>
        </div>
