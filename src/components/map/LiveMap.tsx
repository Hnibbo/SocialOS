...
import type { MapMarker, UserPresence, Group, Activity, Business, LiveStream, Content } from '@/types/social-os';
...
// Dynamic imports for maplibre removed for direct usage to fix UMD/Module issues in this env
...
export function LiveMap({
    ...
    useEffect(() => {
        ...
        const fetchSpatialData = async () => {
            ...
            if (mapRef.current && mapLoaded) {
                const { data: viralData } = await supabase.from('viral_pulse_data' as any).select('*');
                if (viralData) {
                    const viralGeojson = {
                        type: 'FeatureCollection',
                        features: (viralData as any[]).map((v: any) => ({
                            type: 'Feature',
                            geometry: {
                                type: 'Point',
            ...
            const [current, previous] = await Promise.all([
                supabase.from('referrals' as any).select('id', { count: 'exact', head: true }).gt('created_at', thirtyDaysAgo),
                supabase.from('referrals' as any).select('id', { count: 'exact', head: true }).gt('created_at', sixtyDaysAgo).lt('created_at', thirtyDaysAgo)
            ]);
    ...
    // Merge all marker sources with useMemo to optimize performance
    const markers: MapMarker[] = React.useMemo(() => {
        const mergedMarkers: MapMarker[] = [
            ...
            ...streamMarkers,
            ...datingProfiles
                .filter(p => (p as any).location) // Ensure location exists
                .map(p => ({
                    id: (p as any).user_id,
                    type: 'user' as const,
                    coordinates: (p as any).location as [number, number],
                    data: { ...p, isMatch: true } as any
                }))
        ];

        // Filter markers based on visibility settings to avoid unnecessary rendering
        return mergedMarkers.filter(marker => {
            if (marker.type === 'user' && !filters.users) return false;
            if (marker.type === 'group' && !showGroups) return false;
            if (marker.type === 'business' && !filters.businesses) return false;
            if (marker.type === 'activity' && !filters.activities) return false;
            return true;
        }) as MapMarker[];
    }, [entityMarkers, activities, streamMarkers, datingProfiles, filters, showGroups]);
    ...
    const initializeMap = useCallback(async () => {
        ...
        try {
            ...
            map.on('click', async (e) => {
                ...
                const { data } = await supabase.rpc('get_cell_dominance' as any, {
                    p_lat: e.lngLat.lat,
                    p_lng: e.lngLat.lng
                });
                if (data && (data as any).length > 0) {
                    setTerritoryInfo((data as any)[0]);
                } else {
    ...
    }, [latitude, longitude, onMapClick, showVibe, showViral]);
        ...
        <div className={`relative ${className}`}>
            ...
            {mapLoaded && (
                <div className="absolute bottom-32 left-4 right-4 flex justify-center gap-3 overflow-x-auto py-2 no-scrollbar z-20">
                    {[
                        { label: 'People', icon: '👥', active: filters.users, onClick: () => setFilters(prev => ({ ...prev, users: !prev.users })) },
                        { label: 'Events', icon: '🎉', active: filters.activities, onClick: () => setFilters(prev => ({ ...prev, activities: !prev.activities })) },
                        { label: 'Places', icon: '📍', active: filters.businesses, onClick: () => setFilters(prev => ({ ...prev, businesses: !prev.businesses })) },
                        { label: 'Live', icon: '🔴', active: filters.streams, onClick: () => setFilters(prev => ({ ...prev, streams: !prev.streams })) },
                            ...
                            <div className="flex gap-4 items-start mb-6">
                                {selectedEntity.type === 'user' ? (
                                    <div className="w-16 h-16 rounded-full bg-primary/20 flex items-center justify-center border-2 border-primary/50 text-2xl">
                                        {(selectedEntity.data as any).avatar_url ? <img src={(selectedEntity.data as any).avatar_url} className="w-full h-full rounded-full object-cover" /> : '👤'}
                                    </div>
                                ) : (
                                    <div className="w-16 h-16 rounded-[1.5rem] bg-orange-500/20 flex items-center justify-center border-2 border-orange-500/50 text-3xl">
                                        {(selectedEntity.data as any).category === 'drop' ? '⚡' : '🔥'}
                                    </div>
                                )}
                                <div>
                                    <h3 className="text-xl font-black tracking-tighter text-white flex items-center gap-2">
                                        {(selectedEntity.data as any).display_name || (selectedEntity.data as any).title || (selectedEntity.data as any).name || 'Unknown Node'}
                                        {((selectedEntity.data as any).category === 'drop' || (selectedEntity.data as any).category === 'gift') && (
                                    ...
                                    <p className="text-[10px] uppercase font-black tracking-widest text-primary">
                                        {selectedEntity.type} • {(selectedEntity.data as any).category || 'Standard'}
                                    </p>
                                </div>
                            </div>

                            <p className="text-sm text-white/70 mb-6 leading-relaxed">
                                {(selectedEntity.data as any).description || 'No description provided for this proximity node. Tap to interact.'}
                            </p>

                            <div className="grid grid-cols-2 gap-3">
                                <Button
                                    onClick={async () => {
                                        if ((selectedEntity.data as any).category === 'gift') {
                                            const { error } = await supabase.rpc('collect_dropped_asset' as any, { p_asset_id: selectedEntity.id });
                                            if (error) toast.error("Failed to collect");
                                            else toast.success("Asset added to Vault!");
                                            setSelectedEntity(null);
                                        } else if ((selectedEntity.data as any).category === 'drop') {
                                            ...
                                            const { data, error } = await supabase.rpc('collect_moment_drop' as any, {
                                ...
                                >
                                    {((selectedEntity.data as any).category === 'gift' || (selectedEntity.data as any).category === 'drop') ? 'COLLECT' : 'INTERACT'}
                                </Button>
                                <Button
                                    variant="outline"
