...
import { Trophy, Zap, Share2, ChevronRight } from "lucide-react";
...
export default function HupScoreCard() {
    ...
    const progress = Math.min(100, (data.xp / data.nextLevelThreshold) * 100);
            ...
            <div className="relative z-10">
                ...
                <div className="space-y-2 mb-6">
                    <div className="flex justify-between text-xs text-white/50">
                        ...
                        <span>{data.xp} / {data.nextLevelThreshold} XP</span>
                    </div>
                ...
                <Button 
                    className="w-full mt-6 bg-primary/20 hover:bg-primary/30 text-primary border border-primary/50"
                    onClick={() => navigate('/gamification')}
                >
                    View Gamification <ChevronRight className="w-4 h-4 ml-2" />
                </Button>
            </div>
        </div>
    );
