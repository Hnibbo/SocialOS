...
import { useAIModeration, isContentSafe, getViolationReasons } from '@/hooks/useAIModeration';
...
export const CreatePostDialog: React.FC<{ isOpen: boolean; onClose: () => void; onPostAdded?: () => void }> = ({ isOpen, onClose, onPostAdded }) => {
    ...
    const handleCreatePost = async () => {
        ...
        setLoading(true);
        try {
            // 1. Moderate content before publishing
            const moderationResult = await moderate(
                content,
                media.length > 0 ? 'image' : 'text'
            );
            ...
            if (!isContentSafe(moderationResult)) {
                ...
                toast({
                    ...
                    description: `Your post violates our guidelines: ${violationText}`,
                    variant: "destructive",
                });
                setLoading(false);
                return;
