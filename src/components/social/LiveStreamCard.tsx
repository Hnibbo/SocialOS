import { LiveStream } from '@/hooks/useStreaming';
...
export function LiveStreamCard({ stream, onWatch }: LiveStreamCardProps) {
    return (
        <Link to={`/live/${stream.id}`} className="block">
            <Card
                className="w-full max-w-sm overflow-hidden border-0 relative group cursor-pointer transition-all duration-300 hover:scale-[1.02] hover:shadow-xl"
            >
                <div className="aspect-video bg-black relative">
                    ...
                    <div className="absolute bottom-0 left-0 right-0 p-3 bg-gradient-to-t from-black/90 to-transparent">
                        ...
                        <p className="text-gray-300 text-xs flex items-center gap-1 mt-1">
                            ...
                            {stream.host?.full_name || `Creator ${stream.host_id?.slice(0, 4) || 'Unknown'}...`}
                        </p>
                    </div>
