...
export default function SocialGrid() {
    ...
    const drops = markers.filter(m => {
        if (m.type !== 'content') return false;
        const dropData = m.data as { title?: string; type?: string; tags?: string[] };
        if (!userInterests.length) return true;
        return userInterests.some(interest => 
            dropData.title?.toLowerCase().includes(interest.toLowerCase()) ||
            dropData.tags?.some(tag => tag.toLowerCase().includes(interest.toLowerCase()))
        );
    });
                ...
                <div className="flex gap-2 mb-8 overflow-x-auto no-scrollbar pb-2">
                    {['all', 'live', 'drops', 'dominance', 'rooms'].map((tab) => (
                        <button
                            ...
                            className={`px-6 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === tab ? 'bg-primary text-white shadow-lg shadow-primary/30' : 'bg-white/5 text-white border border-white/10'}`}
                        >
                            {tab === 'all' && 'All'}
                            {tab === 'live' && 'Live'}
                            {tab === 'drops' && 'Drops'}
                        ...
                        <div className="space-y-4">
                            {loadingLeaderboard ? (
                                Array.from({ length: 5 }).map((_, i) => (
                                    <div key={i} className="bg-white/5 rounded-2xl p-6 border border-white/10 flex items-center justify-between animate-pulse">
                                        <div className="flex items-center gap-4">
                                            ...
                                            <div className="flex items-center gap-4">
                                                ...
                                                <div className="space-y-2">
                                                    ...
                                                    <Skeleton variant="text" className="w-16 h-2 text-xs" />
                                                </div>
                                            </div>
                                        </div>
                                    ...
                                    <div key={i} className="bg-white/5 rounded-2xl p-6 border border-white/10 animate-pulse">
                                        <div className="flex items-center gap-4 mb-4">
                                            ...
                                            <div className="flex-1 space-y-2">
                                                ...
                                                <Skeleton variant="text" className="w-16 h-3 text-sm" />
                                            </div>
