...
import { Video, Radio, Plus, Share2, Settings } from 'lucide-react';
import SEO from '@/components/SEO';

export default function LiveStreamPage() {
  const { streams, loading, startBroadcast, webrtc, myStream } = useStreaming();
  const [selectedStreamId, setSelectedStreamId] = useState<string | null>(null);
  const [isBroadcasting, setIsBroadcasting] = useState(false);

  const [newStreamTitle, setNewStreamTitle] = useState("");
  const [newStreamDescription, setNewStreamDescription] = useState("");
  const [newStreamVisibility, setNewStreamVisibility] = useState("public");
  ...
  const handleGoLive = async () => {
    ...
    const result = await startBroadcast(newStreamTitle, newStreamDescription, newStreamVisibility);
    if (result) {
  ...
  return (
    <>
      ...
      <div className="p-6 lg:p-8 space-y-8 max-w-7xl mx-auto pb-32">
            ...
            <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
              ...
              <DialogContent className="glass-panel border-white/10 text-white sm:max-w-md">
                <DialogHeader>
                  ...
                  <DialogDescription>
                    Create a new live stream and share your content with the community.
                  </DialogDescription>
                </DialogHeader>
                <div className="space-y-4 py-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <select
                      value={newStreamVisibility}
                      onChange={(e) => setNewStreamVisibility(e.target.value)}
        ...
        {/* Broadcaster View */}
        {isBroadcasting && webrtc.localStream && myStream && (
          <LiveStreamPlayer
