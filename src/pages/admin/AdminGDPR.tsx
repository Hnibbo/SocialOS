    ...
    try {
      ...
      if (selectedRequest.request_type === "export") {
        ...
        const { data: content } = await supabase
          .from("content")
          .select("*")
          .eq("creator_id", selectedRequest.user_id);

        const { data: conversations } = await supabase
          .from("conversations")
          .select("*")
          .or(`user1_id.eq.${selectedRequest.user_id},user2_id.eq.${selectedRequest.user_id}`);
        ...
        await supabase
          ...
          .update({
            ...
            username: `deleted_${selectedRequest.user_id}`,
            bio: null,
            avatar_url: null,
            cover_url: null,
          })
