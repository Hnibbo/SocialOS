...
export function useDating() {
    ...
    const fetchMatches = useCallback(async () => {
        ...
        try {
            const { data, error } = await supabase
                ...
                .select(`
                    *,
                    user_one_profile:user_profiles!matches_user_one_fkey(
                        id, display_name, avatar_url
                    ),
                    user_two_profile:user_profiles!matches_user_two_fkey(
                        id, display_name, avatar_url
                    )
                `)
            ...
            const formattedMatches = data.map(match => {
                const isUserOne = match.user_one === user.id;
                const otherUser = isUserOne ? match.user_two_profile : match.user_one_profile;
                return {
                    id: match.id,
    ...
    const like = useCallback(async (profileId: string) => {
        return swipe(profileId, 'right');
    }, [swipe]);

    const superLike = useCallback(async (profileId: string) => {
        return swipe(profileId, 'super');
    }, [swipe]);

    const pass = useCallback(async (profileId: string) => {
        return swipe(profileId, 'left');
    }, [swipe]);
