...
export function useStreaming() {
    ...
    const createStream = useCallback(async (title: string, description?: string, visibility = 'public') => {
    ...
    const startBroadcast = useCallback(async (title: string, description?: string, visibility = 'public') => {
        const { data: { user } } = await supabase.auth.getUser();
        if (!user) return null;
        ...
        try {
            ...
            const stream = await createStream(title, description, visibility);
            if (!stream) {
                webrtc.stopLocalStream();
                return null;
            }
