-- FORCE UPDATE: RPC Functions (Timestamp bump)
-- This migration guarantees the RPC functions are recreated with correct signatures.
...
-- 1. find_nearby_users (Use 'p_lat', 'p_lng', 'p_radius_meters' to avoid collision with output 'lat')
...
-- 6. update_user_location (Keep as is, no conflict here, but verify signatures)
-- Note: update_user_location does not return a table with column 'lat', so 'lat' input is fine.
-- But for consistency let's use p_lat if needed? No, let's keep it simple as it is just VOID.
...
-- Grant permissions (Update signatures)
