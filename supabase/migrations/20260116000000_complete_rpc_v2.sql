-- Complete RPC Functions Migration v2

-- Drop all existing function variations by exact signature
...
-- Also drop any variations with FLOAT types
...
DROP FUNCTION IF EXISTS public.find_nearby_groups(float, float, integer) CASCADE;
...
DROP FUNCTION IF EXISTS public.update_user_location(numeric, numeric) CASCADE;
...
GRANT EXECUTE ON FUNCTION public.find_nearby_users TO authenticated;
GRANT EXECUTE ON FUNCTION public.find_nearby_activities TO authenticated;
GRANT EXECUTE ON FUNCTION public.find_nearby_groups TO authenticated;
GRANT EXECUTE ON FUNCTION public.find_nearby_drops TO authenticated;
GRANT EXECUTE ON FUNCTION public.find_nearby_assets TO authenticated;
