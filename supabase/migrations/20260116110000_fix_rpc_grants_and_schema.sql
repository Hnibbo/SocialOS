-- FIX: Grant permissions and ensure consistent parameter names
-- This migration ensures that recreated functions have proper GRANTS and unambiguous signatures.

-- 1. Explicitly drop functions we are redefining to avoid ambiguity or zombie signatures
DROP FUNCTION IF EXISTS public.find_nearby_users(double precision, double precision, integer);
DROP FUNCTION IF EXISTS public.find_nearby_activities(double precision, double precision, integer);
DROP FUNCTION IF EXISTS public.find_nearby_groups(double precision, double precision, integer);
...
-- 2. Redefine ALL functions with p_lat/p_lng inputs and location_lat/location_lng outputs
...
-- 3. GRANT EXECUTE Permissions (CRITICAL FOR RPC)
